...
class System:
    ...
    def system_split():
        ...
        for h in System._hardware:
            ...
            result+= f"Hardware Component - {h.name}\nExpress Software Components: {len([s for s in h.software_components if s.type=='Express'])}\n"\
                f"Light Software Components: {len([s for s in h.software_components if s.type=='Light'])}\n"\
                ...
                f"Type: {h.type}\n"\
                f"Software Components: {', '.join([s.name for s in h.software_components]) if ', '.join([s.name for s in h.software_components])  else None}"
        return result
